...
const JotformReviews = lazy(() => import('../components/JotformReviews'));
        ...
        >
          ...
          <div className="flex items-center gap-2 mt-3">
            <div className="flex items-center gap-0.5" aria-label={`${reviewsConfig.rating} van 5 sterren`}>
              {Array.from({ length: 5 }).map((_, i) => {
                const fill = Math.min(1, Math.max(0, reviewsConfig.ratingValue - i));
                return (
                  <span key={i} className="relative inline-block h-5 w-5">
                    <Star className="absolute inset-0 h-5 w-5 text-primary/30" />
            ...
            <span className="text-white font-semibold text-sm">{reviewsConfig.rating}/5 Google Reviews</span>
          </div>
        </PageHero>
